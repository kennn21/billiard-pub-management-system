...
const successToastConfig = {
  ...
  draggable: true,
  progress: undefined,
  icon: <FontAwesomeIcon icon={faPlus} color="#0275d8" />,
};

const warnToastConfig = {
  position: toast.POSITION.BOTTOM_RIGHT,
  autoClose: 3000,
  hideProgressBar: false,
  closeOnClick: true,
  ...
  draggable: true,
  progress: undefined,
  icon: <FontAwesomeIcon icon={faMinus} color="#d9534f" />,
};
...
const handleAddToOrder = (food: Food, quantity: number) => {
  ...
  toast.success(`${food.name} added to the order list!`, successToastConfig);
};
...
const handleRemoveFromOrder = (food: Food, quantity: number) => {
  ...
  if (existingOrder) {
    ...
    toast.warn(`${food.name} removed from the order list!`, warnToastConfig);
  }
};
